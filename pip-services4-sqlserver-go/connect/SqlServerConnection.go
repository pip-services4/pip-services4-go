...
func (c *SqlServerConnection) Open(ctx context.Context) error {

	uri, err := c.ConnectionResolver.Resolve(ctx)
	if err != nil {
		c.Logger.Error(ctx, err, "Failed to resolve SqlServer connection")
		return nil
	}

	c.Logger.Debug(ctx, "Connecting to SqlServer")
	...
	for retries > 0 {
		...
		if err != nil {
			...
			c.Logger.Debug(ctx, "Failed to connect to SqlServers, try reconnect...")
...
func (c *SqlServerConnection) Close(ctx context.Context) error {
	if c.Connection == nil {
		return nil
	}
	c.Connection.Close()
	c.Logger.Debug(ctx, "Disconnected from SqlServer database %s", c.DatabaseName)
	c.Connection = nil
	c.DatabaseName = ""
	return nil
}
...
func (c *SqlServerConnection) waitForRetry(ctx context.Context, retries int) error {

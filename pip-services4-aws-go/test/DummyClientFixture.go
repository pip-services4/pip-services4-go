...
func (c *DummyClientFixture) TestCrudOperations(t *testing.T) {
	...
	dummy1.Id = ckeys.IdGenerator.NextLong()
	dummy2.Id = ckeys.IdGenerator.NextLong()
	...
	// Get all dummies
	dummies, err := c.client.GetDummies(ctx, cquery.NewEmptyFilterParams(), cquery.NewPagingParams(0, 15, false))
	assert.Nil(t, err)
